...
ethernet_device=$(nmcli -t -f DEVICE,TYPE,STATE device status |
  grep -E '^.+:ethernet:connected' |
  awk -F: '{print $1}')

if [ -n "$ethernet_device" ]; then
  ip_address=$(nmcli -t -f IP4.ADDRESS device show "$ethernet_device" | sed -n '1p')
  tooltip="Ethernet Connected\nDevice: $ethernet_device\nIP Address: $ip_address"
  echo "{\"text\": \"󰈀\", \"tooltip\": \"${tooltip}\"}"
  exit 0
fi
